...
大名鼎鼎的PageRank原理其实很简单。利用网页之间的链接关系来确定一个网页重要性，并且使用随机浏览的概念避免不存在外链的Dead End 中rank sink 的问题，同时还可对网页进行个性化推荐[[3]](#3)。    
...
但是，考虑一个简单的情况：当两个或者多个网页互相链接，并且没有其他向外的链接的时候，在对rank 进行迭代时，这几个网页的rank 值会不断增加，引发rank sink 的问题。为了解决这个问题，我们假设用户会在所有网页中随机浏览一些网页（random surfer）。在每次计算的时候我们还需要考虑随机浏览网页的向量，公式如下：      
...
其中$$r_0$$即为Topic-Sensitive PageRank中的v向量，其中每次随机游走有α 的概率回到起始节点。最终我们得到了每个物品和人的rank 如下：
